...
import {
  computed, onBeforeUnmount, onMounted, ref, request, useState, watch,
} from '#imports';
...
const pairedAnswer = useState('pairedAnswer', () => '');
...
const loadQuestions = (excluded: number[] = []) => {
  isRequest.value = true;
  // request.post('questions/game', excluded).then((data) => {
  //   questions.value = [...(excluded ? questions.value : []), ...data];
  // }).finally(() => {
  //   isRequest.value = false;
  // });
  questions.value = [...(excluded ? questions.value : []), ...Array.from({length: 20}, (_, i) => ({id: questions.value.length + i}))];
  isRequest.value = false;
};
...
const onNext = () => {
  ...
  if (questions.value.length < index.value + 3) {
    loadQuestions(questions.value.slice(index.value - 2).map((el) => el.id));
  }
};
...
watch(index, (_, p) => {
  request.post('questions', {
    questionId: questions.value[p].id, answer: answer.value, pairedAnswer: pairedAnswer.value,
  });
});
